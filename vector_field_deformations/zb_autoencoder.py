...
class Zero_bias_AutoEncoder(object):

    def __init__(self, numpy_rng, input=None, n_visible=784, n_hidden=500,
           W=None, symF=True, activation='sig'):
        ...
        if not symF:
            initial_W = numpy.asarray(numpy_rng.uniform(
                      ...
                      high=4 * numpy.sqrt(6. / (n_hidden + n_visible)),
                      size=(n_visible, n_hidden)), dtype=theano.config.floatX)
            R = theano.shared(value=initial_W.T, name='W')
        ...
        if symF:
            ...
            self.W_prime = R
            self.params = [self.W, self.W_prime]

    def get_hidden_values(self, input, thrd=1.0):
        """ Computes the values of the hidden layer """

        #if self.activation == 'relu':
        #    z = T.dot(input, self.W) 
    ...
    def cost(self, X, corrupt_in=0, binary=True, ntype='zeromask'):
        ...
        if binary:
            ...
            L = T.sum(0.5*(X-recondX)*(X-recondX), axis=1)
